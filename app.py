...
# Lottie animations (Professional fitness set)
lottie_workout = load_lottieurl("https://assets5.lottiefiles.com/packages/lf20_tivbc6vo.json") # Workout
lottie_weight = load_lottieurl("https://assets10.lottiefiles.com/packages/lf20_5njp8vtp.json") # Scale
lottie_water = load_lottieurl("https://assets3.lottiefiles.com/packages/lf20_h8sc6oia.json") # Water
lottie_calories = load_lottieurl("https://assets2.lottiefiles.com/packages/lf20_7z2xv9be.json") # Food
...
if choice == "Dashboard":
    ...
    with col2:
        if not calorie_df.empty:
            st.metric("Daily Calories", f"{calorie_df.iloc[-1]['calories']} kcal")
        else:
            st.info("No calorie data")

    with col3:
        if not workout_df.empty:
            st.metric("Total Workouts", f"{len(workout_df)} sessions")
        else:
            st.info("No workout data")

    with col4:
        if not water_df.empty:
            st.metric("Today's Water", f"{water_df.iloc[0]['amount']} ml")
        else:
            st.info("No water data")

    st.markdown("<br>", unsafe_allow_html=True)

    # 2. Goal Progress Bar (Visual Highlight)
    if not weight_df.empty and goal_weight:
        cur_w = weight_df.iloc[-1]['weight']
        goal_w = float(goal_weight)
        start_w = weight_df.iloc[0]['weight']
        
        # Calculate percentage of the way from start to goal
    ...
    with row1_col1:
        ...
        if not weight_df.empty:
            ...
            st.info("Start logging weight to see the trend!")

    with row1_col2:
        st.markdown("### ⚡ Energy")
        if lottie_workout:
            st_lottie(lottie_workout, height=200, key="dash_anim")
    ...
    with col1:
        if lottie_weight:
            st_lottie(lottie_weight, height=250)
        
        with st.form("weight_form"):
            st.markdown("#### Update your weight")
    ...
    with col1:
        if lottie_calories:
            st_lottie(lottie_calories, height=250)
        
        with st.form("calorie_form"):
            st.markdown("#### Log Daily Intake")
    ...
    with col1:
        if lottie_water:
            st_lottie(lottie_water, height=250)
